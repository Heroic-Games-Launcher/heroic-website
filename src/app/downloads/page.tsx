      ...
      <div className='flex flex-col lg:flex-row gap-4 justify-between mt-10'>
        {
          downloadSection.map(({ name, desc, image, url }) => (
            <section key={name} className='w-full flex flex-col justify-between gap-2 items-center p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700'>
              <h2 className='mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white'>{name}</h2>
              <Image src={image} priority alt='Windows logo' width={100} height={100} />
              <p className='mb-3 text-center font-normal text-gray-700 dark:text-gray-400'>{desc}</p>
              <div className='flex gap-2 flex-wrap justify-center'>
                {
                 Object.entries(url).map(([name, value]) => (
                   // TODO: fix type
                   <a key={name} href={data[name]} className='flex gap-2 w-fit items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'>
                     {value}
