...
export async function getLatestReleases(): Promise<ReleaseUrls> {
  ...
  const debImage = assets.filter((a) => a.name.endsWith('.deb'))[0]
    ?.browser_download_url
  const rpmImage = assets.filter((a) => a.name.endsWith('.rpm'))[0]
    ?.browser_download_url
  const windowsSetupStable = assets.filter((a) =>
    a.name.includes('Setup-x64')
  )[0]?.browser_download_url
  const windowsSetupStableArm64 = assets.filter((a) =>
    a.name.includes('Setup-arm64')
  )[0]?.browser_download_url
  const dmgStable = assets.filter((a) => a.name.includes('macOS-x64'))[0]
    ?.browser_download_url
  const dmgStableArm64 = assets.filter((a) => a.name.includes('macOS-arm64'))[0]
    ?.browser_download_url
